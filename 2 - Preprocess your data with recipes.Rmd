...
Tutorial link: https://www.tidymodels.org/start/recipes/

## Introduction

In part 1 we learned how to specify and train models with different engines using the `parsnip` package. In this article, we’ll explore another tidymodels package, `recipes`, which is designed to help you preprocess your data before training your model. Recipes are built as a series of preprocessing steps, such as:
...
* With `step_rm()`, we remove the original `date` variable since we no longer want it in the 
...
Let’s use logistic regression to model the flight data. As we saw in _Build a Model_, we start by building a model specification using the `parsnip` package:
